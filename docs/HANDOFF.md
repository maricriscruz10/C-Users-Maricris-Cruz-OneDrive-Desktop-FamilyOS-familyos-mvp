...
| C1 — Recurring chore auto-advance | ✅ Done (2026-06-24) | Completing a recurring chore no longer leaves it stuck at the same due date. Added `advanceDueDate(fromDate, recurrence)` helper in `backend/server.js`. `POST /api/chores/:id/complete` now advances `due_date` by the chore's period (daily +1d, weekly +7d, monthly +1mo) instead of leaving it unchanged. Non-recurring chores still flip to `status='completed'`. |
| C2 — Event search & filter | ✅ Done (2026-06-24) | New 🔍 Search screen in the sidebar (visible to all roles). Fields: keyword (title/location/provider), event type (all/general/appointment), from/to date range. Results grouped by date with day-headers; each result row opens the event edit modal on click. Search is triggered by the Search button or pressing Enter in the keyword field. The backend date-range and category filters are used server-side; keyword filtering is done client-side. Added `.event-date-header` CSS class to `web/styles.css`. |
| C3 — Server-side event reminders | ✅ Done (2026-06-24) | `setInterval` scheduler in `backend/server.js` runs every 60s. For each family, reads `reminderMinutesBefore` from `settings_json` (default 60). Finds events (incl. recurring occurrences via `expandOccurrences`) whose `startAt` falls within a ±1-minute window of `now + reminderMinutes`. Creates an `event_reminder` notification for each assignee and fires an Expo push if they have a push token. Dedup: in-memory `sentReminders` Set (key = `eventId:userId:startAt`) prevents re-sending within a server session. Configured via the Settings screen's "Reminder" field. |
| C4 — Family activity feed | ✅ Done (2026-06-24) | New `GET /api/families/:id/activity` backend endpoint (accessible to all roles) returns the last 20 audit events with actor name/avatar but without raw before/after JSON. New `activityFeedWidget()` renders a "Recent activity" card at the bottom of the Dashboard showing who did what and when (time-ago labels). Falls back to empty string silently if the fetch fails. | `setInterval` scheduler in `backend/server.js` runs every 60s. For each family, reads `reminderMinutesBefore` from `settings_json` (default 60). Finds events (incl. recurring occurrences via `expandOccurrences`) whose `startAt` falls within a ±1-minute window of `now + reminderMinutes`. Creates an `event_reminder` notification for each assignee and fires an Expo push if they have a push token. Dedup: in-memory `sentReminders` Set (key = `eventId:userId:startAt`) prevents re-sending within a server session. Configured via the Settings screen's "Reminder" field. |

| C5 — Shopping list from meal plan | ✅ Done (2026-06-24) | "🛒 Shopping list" button added to the Meals screen header (visible to all roles). Opens a modal showing the planned meals grouped by day. Includes a "Copy as text" button that copies a plain-text shopping list to the clipboard (with `navigator.clipboard` + `execCommand` fallback). No new API call — uses the already-loaded `state.meals`. |
| C6 — Dark mode | ✅ Done (2026-06-24) | Dark theme implemented via `[data-theme="dark"]` CSS overrides on the `:root` variables. Preference stored in `localStorage` (`familyos_dark=1`). `applyTheme()` called on page load. Toggle button ("🌙 Dark mode" / "☀️ Light mode") added to the bottom of the sidebar. Switches immediately without a page reload; button label updates in-place. |
| C7 — iCal export | ✅ Done (2026-06-24) | "⬇ Export .ics" button added to Calendar screen. `exportIcal(events)` builds a RFC-5545 iCalendar string from the current month's events (VTIMEZONE not included; DTSTART/DTEND are UTC ISO8601). Downloads as `familyos-YYYY-MM.ics`. Pure client-side — no server endpoint. Long property values are line-folded per RFC. |
| C8 — Budget spending chart (pure CSS) | ✅ Done (2026-06-24) | Added a "Spending by category" card in the Budget screen between the stat cards and the transactions table. Each category gets a labeled horizontal bar: fill width = `(spent / limit) * 100%`, capped at 100%. Bar color matches the category color; switches to `var(--red)` when over budget. CSS classes `.budget-bar-track` / `.budget-bar-fill` with a 0.4s width transition. No canvas or external library. |
| C9 — Chore streaks & history | ✅ Done (2026-06-24) | New `GET /api/families/:id/chores/streaks` endpoint returns: (1) per-user streak (consecutive days with at least one completion, ending today or yesterday), (2) last 15 completions across the family. Chores screen now shows a "Streaks 🔥" card (hidden when nobody has a streak) and a "Completion history" card with completer name, chore title, date, and points. | Added a "Spending by category" card in the Budget screen between the stat cards and the transactions table. Each category gets a labeled horizontal bar: fill width = `(spent / limit) * 100%`, capped at 100%. Bar color matches the category color; switches to `var(--red)` when over budget. CSS classes `.budget-bar-track` / `.budget-bar-fill` with a 0.4s width transition. No canvas or external library. | "⬇ Export .ics" button added to Calendar screen. `exportIcal(events)` builds a RFC-5545 iCalendar string from the current month's events (VTIMEZONE not included; DTSTART/DTEND are UTC ISO8601). Downloads as `familyos-YYYY-MM.ics`. Pure client-side — no server endpoint. Long property values are line-folded per RFC. | Dark theme implemented via `[data-theme="dark"]` CSS overrides on the `:root` variables. Preference stored in `localStorage` (`familyos_dark=1`). `applyTheme()` called on page load. Toggle button ("🌙 Dark mode" / "☀️ Light mode") added to the bottom of the sidebar. Switches immediately without a page reload; button label updates in-place. | "🛒 Shopping list" button added to the Meals screen header (visible to all roles). Opens a modal showing the planned meals grouped by day. Includes a "Copy as text" button that copies a plain-text shopping list to the clipboard (with `navigator.clipboard` + `execCommand` fallback). No new API call — uses the already-loaded `state.meals`. |

| C10 — AI event parsing | ⏳ Pending (needs Claude API key) | Will add a "Parse with AI" button to the event creation modal. Sends natural-language input (e.g., "dentist Tuesday at 3pm") to the Claude API, which returns structured JSON (`title`, `startAt`, `endAt`, `location`, `category`). Pre-fills the event form fields. Requires `CLAUDE_API_KEY` env var (backend) and a new `POST /api/families/:id/events/parse` endpoint. |
